...
A `CancellationToken` implementation that provides a shared cancellation signal
for coordinating the cancellation of multiple independent promises and async
operations from a single control point. Complements the built-in cancellation
on `hiblaphp/promise` by adding external, user-driven cancellation that can
span across unrelated promise chains.
...
`hiblaphp/promise` has built-in cancellation on every promise — `cancel()`,
`cancelChain()`, and `onCancel()` are all first-class features of the promise
itself. That system is self-contained: cleanup is registered at the point where
the resource is created, and cancellation propagates through the chain
automatically.

`hiblaphp/cancellation` sits on top of that. It provides an external shared
signal — a `CancellationToken` — that you can pass into any number of
independent operations. When you cancel the token source, every operation
holding that token is cancelled together, triggering their individual
`onCancel()` handlers.

The two systems compose naturally:
```
CancellationTokenSource::cancel()
  -> token->track($promise) links signal to promise
  -> Promise::cancel() fires
  -> Promise::onCancel() handlers fire synchronously
  -> Resources are freed
```
...
The library follows a producer/consumer split. You hold the
`CancellationTokenSource` — it owns the ability to cancel. You pass the
`CancellationToken` into operations — it is a read-only view of the
cancellation signal. Operations never receive the source, only the token.
...
`CancellationToken` and use it to either check for cancellation at
safe checkpoints or to track the promises they create.

### Polling with `isCancelled()`

Use `isCancelled()` for non-throwing checks inside loops or before
...
Use `track()` to register a promise for automatic cancellation when
the token is cancelled. The promise is automatically untracked when it
settles — no manual cleanup needed. Tracking an already-settled or
already-cancelled promise is a safe no-op — `track()` checks the promise
state and returns it immediately if it has already completed:
```php
use Hibla\Cancellation\CancellationToken;
...
`CancellationToken::none()` returns a shared singleton token that can
never be cancelled. It is safe to pass across multiple callers simultaneously
since it never fires. Use it as a default parameter value — this lets you
call `isCancelled()` and `throwIfCancelled()` on it safely without any null
checks:
```php
use Hibla\Cancellation\CancellationToken;
...
function fetchUser(
    ...
    $token ??= CancellationToken::none(); // safe singleton — never cancels
...
If any `onCancel()` callback or promise cancellation throws an exception
during `cancel()`, the library does not abort mid-loop. All remaining
callbacks and promises are still processed. Exceptions are collected and
at the end either a single exception is re-thrown or — if multiple callbacks
threw — an `AggregateErrorException` is thrown containing all of them.
...
try {
    ...
    // contains all exceptions from all callbacks that threw
...
## 6. Cleanup Registration with `onCancel()`

`onCancel()` on the token itself is for registering cleanup that is not
tied to a specific promise — for example logging, releasing a lock, or
updating state when any operation in a group is cancelled.

It returns a `CancellationTokenRegistration` that lets you unregister the
callback if the operation completes before cancellation occurs.
`dispose()` returns `true` if the callback was successfully removed, `false`
if it was already disposed or had already fired:
```php
$cts   = new CancellationTokenSource();
$token = $cts->token;
...
// Operation cancels if user cancels OR timeout expires — whichever comes first
...
`await()` from `hiblaphp/async` accepts an optional `CancellationToken`
as its second argument. When provided, it automatically calls
...
> the internal state. Promises tracked on the token are untracked but not
> cancelled. If you need promises to be cancelled when the source goes out
> of scope, call `cancel()` explicitly in a `finally` block:
```php
function doWork(): mixed
{
    ...
    try {
        ...
        // and their onCancel() handlers fire before $cts is destroyed.
        // Without this, tracked promises continue running after scope exit.
...
The timeout timer is always cancelled automatically on scope exit — no
manual cleanup needed for the timer itself.
...
| `cancel()` | Cancel synchronously. Calls `Promise::cancel()` (forward-only) on all tracked promises. Idempotent — safe to call multiple times. Collects and throws `AggregateErrorException` if multiple callbacks throw. |
...
| `cancelAfter(float $seconds)` | Set or reset the automatic cancellation timer. Safe to call multiple times — resets on each call. |
...
| `onCancel(callable $callback): CancellationTokenRegistration` | Register a synchronous cleanup callback. Must be fast — no blocking or awaiting. Returns a registration for unregistering. If already cancelled, fires immediately and returns a pre-disposed registration. |
| `track(PromiseInterface $promise): PromiseInterface` | Register a promise for automatic cancellation. Auto-untracked when promise settles. Safe no-op on already-settled or already-cancelled promises. Returns the same promise. |
...
| `CancellationToken::none()` | Static singleton. A token that can never be cancelled. Safe to share across multiple callers. Use as a default parameter. |
